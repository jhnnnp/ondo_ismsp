...
def test_workspace_html_has_access_gate_and_noindex() -> None:
    ...
    assert "restoreWorkspaceShell(routeId)" in router
    assert router.index("restoreWorkspaceShell(routeId)") < router.index(
      "await ensureWorkspaceAccess()",
    )


def test_workspace_has_loading_skeleton_and_current_bundle() -> None:
    response = client.get("/workspace")
    ...
    assert "/controls/map/assets/react-dist/workspace.js?v=20260820-12" in response.text
    assert "/controls/map/assets/control_map.css?v=20260820-18" in response.text
    ...
    assert 'classList.add("is-workspace-loading")' in analysis_controller
    layout_css = (WEB_ROOT / "styles" / "layout.css").read_text(encoding="utf-8")
    assessment_css = (WEB_ROOT / "styles" / "assessment.css").read_text(encoding="utf-8")
    assert "#view-analyze.is-workspace-loading #analysisReportPanel" in layout_css
    assert "#view-analyze.is-workspace-loading.is-report #analysisReportPanel" in layout_css
    assert "#view-analyze.is-report:not(.is-workspace-loading) #analysisReportPanel" in assessment_css
    assert 'class="workspace-skeleton-assessment"' in response.text
    assert 'class="workspace-skeleton-report"' in response.text
    assert "#view-analyze.is-report .workspace-skeleton-report" in layout_css
    assert '보고서를 불러오는 중' in router
