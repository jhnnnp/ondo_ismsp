...
`).trim()}function Fa(e){if(!/여부와/.test(e))return[e];const t=e.split(/\s*여부와\s+/);if(t.length!==2)return[e];const s=t[0].trim(),n=t[1].trim();return!s||!n?[e]:!/(여부|있는지|할지|인지|「)/.test(s)||!/(여부|「)/.test(n)?[e]:[/여부\??$/.test(s)?s:`${s} 여부`,n]}function za(e){if(e.length<160)return[e];const t=e.split(new RegExp("(?<=(?:합니다|됩니다|있습니다|없습니다|봅니다|판단됩니다|어렵습니다|해당하지 않습니다|요청함)\\.)\\s+"));return t.length>1?t.map(s=>s.trim()).filter(Boolean):[e]}function _a(e,t){return/^(행위 주체 내용|관리주체|질의 배경|관계 법령|결론)$/.test(e)||e.length<=40&&/(?:결정례|판례|구조도|법적 근거)$/.test(e)?!0:e.length<=16&&!/[.!?。?]$/.test(e)&&!/^[①-⑳가나다라마바사\d]/.test(e)&&/^[①-⑳]/.test(t||"")}function Tt(e){const t=Ha(e).split(/\n+/).map(u=>u.trim()).filter(u=>u&&!/^\d+\s*[•·]\s*$/.test(u)&&!/^[ⅠⅡⅢⅣⅤⅥⅦⅧⅨⅩ]+$/.test(u)).flatMap(u=>/^([가나다라마바사아자차카타파하]\.|[①-⑳]|\d{1,2}\.|-|※|<)/.test(u)?[u]:za(u));if(!t.length)return"";const s=[];let n=[],a=[],r="";const o=()=>{n.length&&(s.push(`<p>${c(xe(n.join(" ")))}</p>`),n=[])},l=()=>{if(!a.length)return;const u=r==="hangul",f=r==="bullet",v=f?"ul":"ol",g=u?"legal-hangul-list":f?"legal-bullet-list":"legal-reasoning-list";s.push(`<${v} class="${g}">${a.map(h=>`<li>${c(xe(h))}</li>`).join("")}</${v}>`),a=[],r=""},d=(u,f)=>{r&&r!==u&&l(),r=u,a.push(f)};return t.forEach((u,f)=>{const v=u.replace(/^<\s*([^>]+)\s*>$/,"$1").trim(),g=/^<\s*([^>]+)\s*>$/.test(u),h=v.match(/^([가나다라마바사아자차카타파하])\.\s*(.+)$/),b=v.match(/^(\d{1,2})\.\s+(.+)$/),A=v.match(/^([①-⑳])\s*(.+)$/),E=v.match(/^-\s*(.+)$/),y=v.match(/^※\s*(.+)$/),x=/^(?:[①-⑳]|\d{1,2}\.)\s*[∼~～]/.test(v),m=t[f+1]||"";if(x){o(),l(),s.push(`<p class="legal-note">${c(xe(v))}</p>`);return}if(g||_a(v,m)){o(),l(),s.push(`<h5>${c(xe(v))}</h5>`);return}if(y){o(),l(),s.push(`<p class="legal-note">${c(xe(y[1]))}</p>`);return}if(h){o(),d("hangul",h[2]);return}if(b||A||E){o(),d(E?"bullet":"decimal",(b||A||E)[b||A?2:1]);return}const S=Fa(v);if(S.length>1){o(),S.forEach(w=>d("decimal",w));return}if(a.length){const w=a[a.length-1];if(!(/[.)”’]$/.test(w)&&/^(이 |그 |따라서|그러므로|한편|반면|결론적으로|대법원|헌법재판소)/.test(v))){a[a.length-1]=`${w} ${v}`;return}}l(),n.push(v),(/[.!?。?][”’)]?$/.test(v)||/(?:합니다|됩니다|있습니다|없습니다|봅니다|판단됩니다|요청함)[.]?$/.test(v))&&o()}),o(),l(),`<div class="legal-prose legal-reasoning-content">${s.join("")}</div>`}function Ie(e,t,s){const n=Tt(t);return n?`<section class="legal-reading-section is-${s}"><h4>${c(e)}</h4>${n}</section>`:""}function Wt(e,t=i.legalBasisCache?.[e]){if(!t||t.status==="idle")return'<p class="today-detail-note">관련 법령과 법령해석을 불러오는 중입니다.</p>';if(t.status==="loading")return'<p class="today-detail-note" role="status">법적 근거를 불러오는 중...</p>';if(t.status==="error")return`
  ...
  `}function Ua(){const e=qa(),t=Da(),s=p("areaFilterGroup");if(s){const r=[{id:"all",label:"전체"},{id:"1",label:J[1]},{id:"2",label:J[2]},{id:"3",label:J[3]}];s.innerHTML=r.map(o=>{const l=o.id==="all"?e.all:e[o.id]||0,d=o.id==="all"?`전체 ${l}개 통제`:`${t[o.id]||o.label} (${l}개)`;return`
    ...
    `).join("")}}function Ga(e,{ensureChecks:t,ensureDomainChecks:s}){const n=L(e.id),a=t(e.id),r=i.inputConfidence?.[e.id]||"unknown",o=i.expandedRows.has(e.id),l=e.checklistItems||[],d=o?s(e.id,l):i.domainChecks[e.id]||{},u=l.map((y,x)=>{const m=String(x+1),S=!!d[m];return o?`
      ...
      `:`<li><strong>${m}.</strong> ${c(y)}</li>`}).join(""),f=(e.recommendedActions||[]).map(y=>`<li>${c(y)}</li>`).join(""),v=(e.officialEvidenceExamples||[]).slice(0,6).map(y=>`<li>${c(y)}</li>`).join(""),g=zt(e.id),h=g.length?g.map(y=>`<li><strong>${c(y.title)}</strong>${y.url?` · ${c(y.url)}`:""}${y.note?` · ${c(y.note)}`:""}</li>`).join(""):"<li>등록된 증적 없음 (이행 판정에 필요)</li>",b=n==="evidenced"?"done":n,A=e.officialRequirement?`<div class="detail-block"><h3>인증기준 (안내서)</h3><p>${c(e.officialRequirement)}</p></div>`:"",E=o?`<div class="detail-block legal-basis-block">
          <h3>법적 근거 및 참고자료</h3>
  ...
  `,n.querySelector("#resetAssessFiltersBtn")?.addEventListener("click",()=>{s()})}}function Ye(e,t){const s=p("assessCategoryNav");if(!s)return;const n=cn();if(!e.length){s.innerHTML='<p class="detail-empty">표시할 분류가 없습니다.</p>';return}const a=new Map;e.forEach(o=>{const l=o.areaId||"0";a.has(l)||a.set(l,{areaId:l,areaName:o.areaName||J[l]||"기타",groups:[]}),a.get(l).groups.push(o)});const r=Array.from(a.values()).sort((o,l)=>ke(o.areaId,l.areaId));s.innerHTML=r.map(o=>{const l=o.groups.reduce((f,v)=>f+v.controls.filter(g=>t.has(g.id)).length,0),d=o.groups.reduce((f,v)=>f+v.controls.length,0),u=n?`${l}/${d}`:String(d);return`
    <div class="assess-area-block">
      <div class="assess-area-label">
        <span>${J[o.areaId]||o.areaName}</span>
        <span>${u}</span>
      </div>
      ${o.groups.map(f=>{const v=f.controls.filter(E=>t.has(E.id)).length,g=nt(f.controls),h=n&&v===0,b=i.activeCategoryId===f.categoryId?" active":"",A=n?`${v}/${f.controls.length}`:`${g.reviewed}/${g.total} (${g.pct}%)`;return`
  ...
  `}function vi(){const e=(i.checklist||[]).filter(s=>L(s.id)!=="na");return{reviewed:e.filter(s=>{const n=L(s.id);return n!=="unknown"&&n!=="na"}).length,applicable:e.length}}function Ge(){return(i.checklist||[]).filter(e=>L(e.id)!=="na")}function Cn(){return Ge().filter(e=>L(e.id)==="unknown")}function An(){return Ge().filter(e=>{const t=L(e.id);return t==="done"||t==="evidenced"})}function En(){const e=new Set(["unknown","none","partial"]);return Ge().filter(t=>e.has(L(t.id)))}function hi(e=[]){const t=new Set(e),s={unknown:0,none:1,partial:2};return En().filter(n=>!t.has(n.id)).sort((n,a)=>(s[L(n.id)]??9)-(s[L(a.id)]??9)||String(n.id).localeCompare(String(a.id)))}function Oe(){return xt(Ge())}function ms(e=null){const t=En().sort((a,r)=>ke(a.id,r.id));if(!t.length)return null;if(!e)return t[0].id;const s=t.findIndex(a=>a.id===e);return s>=0&&s+1<t.length?t[s+1].id:t.find(a=>ke(a.id,e)>0)?.id||t[0].id}function rt(){return Ge().slice().sort((e,t)=>ke(e.id,t.id)).map(e=>e.id)}function Mt(e,t=1){const s=rt();if(!s.length)return null;const n=s.indexOf(e);if(n<0)return t>0?s[0]:s[s.length-1];const a=n+t;return a<0||a>=s.length?null:s[a]}const vs={critical:"심각도 매우 높음",high:"심각도 높음",medium:"심각도 보통"},hs=[{min:80,key:"ready",label:"준비 완료",hint:"핵심 영역이 준비 완료 구간에 들어왔습니다."},{min:55,key:"rising",label:"안정화",hint:"핵심 영역이 안정화되고 있습니다. 남은 보완 항목을 이어서 확인하세요."},{min:25,key:"warming",label:"점검 중",hint:"진단이 진행 중입니다. 다음 통제를 점검하면 온도가 올라갑니다."},{min:0,key:"cold",label:"초기 단계",hint:"아직 점검 초반입니다. 미점검 통제부터 이어서 확인하세요."}];function yi(e={}){const t=Object.values(e).filter(Array.isArray);return{itemCount:t.reduce((s,n)=>s+n.length,0),controlCount:t.filter(s=>s.length>0).length}}function bi(e){return e==="done"||e==="evidenced"?1:e==="partial"?.5:0}function ae(e){const t=Number(e);return Number.isFinite(t)?Math.max(0,Math.min(100,Math.round(t))):0}function Ln({done:e=0,partial:t=0,applicable:s=0}={}){return s<=0?0:ae((Number(e)+Number(t)*.5)/s*100)}function wi({done:e=0,partial:t=0,none:s=0,applicable:n=0}={}){const a=Math.max(0,Number(e)+Number(t)+Number(s)),r=Math.max(0,Number(n)),o=r?ae(a/r*100):0,l=a?ae((Number(e)+Number(t)*.5)/a*100):null;return{reviewed:a,total:r,coverage:o,confirmedReadiness:l,confidence:o>=80?"high":o>=40?"medium":"low"}}function Yt(e){const t=ae(e);return hs.find(s=>t>=s.min)||hs.at(-1)}function $i(e=[],t=()=>"unknown"){const s=new Map;return e.forEach(n=>{const a=String(n.areaId||n.categoryId?.split(".")[0]||"0");s.has(a)||s.set(a,{areaId:a,label:J[a]||n.areaName||`영역 ${a}`,controls:[]}),s.get(a).controls.push(...n.controls||[])}),Array.from(s.values()).sort((n,a)=>String(n.areaId).localeCompare(String(a.areaId),"en",{numeric:!0})).map(n=>{const a=n.controls.length,r=n.controls.filter(f=>{const v=t(f.id);return v!=="unknown"&&v!=="na"}).length,o=n.controls.reduce((f,v)=>f+bi(t(v.id)),0),l=a?ae(o/a*100):0,d=r?ae(o/r*100):null,u=n.controls.find(f=>["unknown","none","partial"].includes(t(f.id)));return{areaId:n.areaId,label:n.label,temperature:l,confirmedReadiness:d,coverage:a?ae(r/a*100):0,band:Yt(l),reviewed:r,total:a,nextControlId:u?.id||""}})}function Si(e={}){const t=[];e.scenarioRelevant&&t.push("선택 시나리오 직접 관련"),e.level==="none"?t.push("미이행 우선"):e.level==="partial"&&t.push("부분 이행 보완 필요"),vs[e.severity]&&t.push(vs[e.severity]);const s=Array.isArray(e.profileRelevance)?e.profileRelevance[0]:"";return s&&t.push(`조직 조건 반영: ${s}`),t.slice(0,3)}function ki(e,t=null,s=[]){const n=t||s[0]||null;return e.levelFilter="weak",e.sessionSelectedControlId=n,n}function Ci({analysis:e,controlEvidence:t={},weakControlIds:s=[],stale:n=!1,groups:a=[],getLevel:r=()=>"unknown",nextControls:o=[],done:l=0,partial:d=0,applicable:u=0}={}){const f=yi(t),v=[...new Set(s.map(String))],g=v.filter(w=>Array.isArray(t[w])&&t[w].length>0).length,h=Math.max(v.length-g,0),b=Ln({done:l,partial:d,applicable:u}),A=wi({done:l,partial:d,none:v.filter(w=>r(w)==="none").length,applicable:u}),E=Yt(b),y=$i(a,r),x=[...y].sort((w,k)=>w.temperature-k.temperature)[0]||null,m=n?[]:(e?.topGaps||[]).slice(0,3).map((w,k)=>({rank:k+1,controlId:w.controlId||w.id||"",title:w.title||w.controlTitle||"우선 보완 통제",risk:w.riskIfMissing||"미흡 시 영향을 확인하고 보완 계획을 수립하세요.",selectionReasons:Si(w),mode:"gap"})),S=m.length?[]:o.slice(0,3).map((w,k)=>{const I=w.level||r(w.id)||"unknown";return{rank:k+1,controlId:w.id||w.controlId||"",title:w.title||"다음 점검 통제",risk:"이 통제를 진단하면 준비 온도가 올라갑니다.",selectionReasons:[_[I]||"미점검","다음 점검 대상"],mode:"queue",weak:I==="none"||I==="partial"}});return{stale:n,temperature:b,band:E,summary:A,areas:y,coolest:x,queueMode:!m.length,priorities:m.length?m:S,evidence:f,signals:[{label:"증적 미등록",value:h,suffix:"개",help:`보완 대상 ${v.length}개 중 ${g}개 통제에 증적이 등록되어 있습니다.`},{label:"등록 증적",value:f.itemCount,suffix:"건",help:f.itemCount?`${f.controlCount}개 통제에 증적이 연결되어 있습니다.`:"아직 등록된 증적이 없습니다. 판단 근거를 남기면 여기에 모입니다."}],categories:n?[]:(e?.gapClusters||[]).slice(0,4).map(w=>({label:w.theme||"미흡 통제 묶음",gapCount:Number(w.gapCount||0),controlId:String(w.controlIds?.[0]||w.primaryControl?.controlId||"")}))}}const Ai=[{id:"unknown",label:"미확인"},{id:"none",label:"미이행"},{id:"partial",label:"부분 이행"},{id:"done",label:"이행"}];function Ei({remaining:e=0,remediationCount:t=0}={}){return e>0?{title:"아직 진단하지 않은 통제가 있습니다.",help:"미점검 항목을 이어서 진단하면 준비 온도와 보완 대상이 더 정확해집니다.",actionAttr:"data-progress-next",actionLabel:"다음 미점검 통제",recCount:e,recLabel:"미점검 대상",recHelp:"우선순위가 높은 통제부터 현재 운영 상태를 확인하세요.",recItems:["미점검 통제 진단하기","판단 근거가 되는 증적 연결"]}:t>0?{title:"보완이 필요한 통제가 있습니다.",help:"부분 이행 항목을 중심으로 증적과 조치 상태를 우선 검토하세요.",actionAttr:"data-progress-weak",actionLabel:"우선 통제 검토",recCount:t,recLabel:"보완 대상",recHelp:"우선순위가 높은 통제부터 증적과 조치 상태를 확인하세요.",recItems:["부족한 증적 보완","개선 조치 계획 수립"]}:{title:"준비 상태가 안정권에 도달했습니다.",help:"적용 통제 진단을 모두 반영했습니다. 증적과 보고서를 이어서 확인하세요.",actionAttr:"",actionLabel:"",recCount:0,recLabel:"보완 대상 없음",recHelp:"등록한 증적과 보고서를 이어서 확인하세요.",recItems:["등록 증적 점검","진단 보고서 확인"]}}let ys=!1;function Li(){return i.levelFilter!=="all"||!!i.assessSearch.trim()||i.areaFilter!=="all"}function xn(e){const t=i.assessSearch.trim().toLowerCase();return e.map(s=>({...s,controls:s.controls.filter(n=>{if(i.areaFilter!=="all"&&String(n.areaId)!==String(i.areaFilter))return!1;const a=L(n.id);return i.levelFilter==="weak"&&!["none","partial"].includes(a)||!["all","weak"].includes(i.levelFilter)&&a!==i.levelFilter?!1:t?on(n,i.assessSearch):!0})})).map(s=>({...s,controls:ln(s.controls,i.assessSearch)})).filter(s=>s.controls.length)}function xi(){document.querySelectorAll("[data-session-level]").forEach(t=>{const s=t.getAttribute("data-session-level")===(i.levelFilter||"all");t.classList.toggle("is-active",s)});const e=p("sessionSearch");e&&e.value!==(i.assessSearch||"")&&(e.value=i.assessSearch||"")}function De(){if(!i.analysis)return;const e=F._hooks||{};Pn(i.analysis,{diagnoseControl:e.diagnoseControl,markAnalysisStale:e.markAnalysisStale})}function Ii(){if(ys)return;const e=p("sessionMasterTools");e&&(ys=!0,p("sessionSearch")?.addEventListener("input",t=>{i.assessSearch=t.target.value||"",De()}),e.querySelectorAll("[data-session-level]").forEach(t=>{t.addEventListener("click",()=>{const s=t.getAttribute("data-session-level")||"all";i.levelFilter=s,De()})}),e.querySelectorAll("[data-session-bulk-preset]").forEach(t=>{t.addEventListener("click",()=>{const s=t.getAttribute("data-session-bulk-preset");s&&(ri(s),De(),F._hooks?.markAnalysisStale?.())})}))}function Qt(e=[]){const t=new Map;return e.forEach(s=>{s?.controlId&&(s.slotId||String(s.actionId||"").startsWith("evidence-")||t.has(s.controlId)||t.set(s.controlId,s))}),t}function In(e=[]){return e.filter(t=>{if(!t?.controlId||t.slotId||String(t.actionId||"").startsWith("evidence-"))return!1;const s=L(t.controlId);return s!=="done"&&s!=="evidenced"&&s!=="na"}).slice(0,10).map(t=>t.controlId)}function Ti(e,t){const s=new Set(t.flatMap(n=>n.controls.map(a=>a.id)));return e&&s.has(e)?e:t[0]?.controls?.[0]?.id||null}function be(e){i.sessionSelectedControlId=e||null,Ue()}function Ri(e,t){i.sessionCollapsedCategories instanceof Set||(i.sessionCollapsedCategories=new Set),!i.sessionCollapsedCategories.size&&t.length&&t.forEach(n=>i.sessionCollapsedCategories.add(n.categoryId));const s=t.find(n=>n.controls.some(a=>a.id===e));s&&i.sessionCollapsedCategories.delete(s.categoryId)}function Pi(e){return e==="done"||e==="evidenced"?"done":e==="none"?"none":e==="partial"?"partial":"unknown"}function Mi(e,t,s){const n=p("sessionMasterTree"),a=p("sessionMasterCount");if(!n)return;const r=new Map;e.forEach(u=>{const f=String(u.areaId||u.categoryId?.split(".")[0]||"0");r.has(f)||r.set(f,{areaId:f,areaName:J[f]||u.areaName||`영역 ${f}`,groups:[]}),r.get(f).groups.push(u)});const o=Array.from(r.values()).sort((u,f)=>ke(u.areaId,f.areaId)),l=e.reduce((u,f)=>u+f.controls.length,0),d=Cn().length;a&&(a.textContent=Li()?`표시 ${l} · 미완료 ${d}`:`미완료 ${d}`),n.innerHTML=o.map(u=>{const f=u.groups.flatMap(g=>g.controls),v=nt(f);return`
        ...
        ${u.groups.map(g=>{const h=i.sessionCollapsedCategories.has(g.categoryId),b=nt(g.controls);return`
            <div class="session-cat${h?" is-collapsed":""}" data-session-cat="${c(g.categoryId)}">
              ...
              <div class="session-cat-items">
                ${g.controls.map(A=>{const E=L(A.id),y=Pi(E),x=A.id===t?" is-selected":"",m=s.has(A.id)?" is-priority":"";return`
    ...
    `}).join("")||'<p class="detail-empty">표시할 통제가 없습니다.</p>',n.querySelectorAll("[data-toggle-session-cat]").forEach(u=>{u.addEventListener("click",()=>{const f=u.getAttribute("data-toggle-session-cat");i.sessionCollapsedCategories.has(f)?i.sessionCollapsedCategories.delete(f):i.sessionCollapsedCategories.add(f);const v=n.querySelector(`[data-session-cat="${f}"]`),g=i.sessionCollapsedCategories.has(f);v?.classList.toggle("is-collapsed",g),u.setAttribute("aria-expanded",g?"false":"true")})}),n.querySelectorAll("[data-select-control]").forEach(u=>{u.addEventListener("click",()=>{Tn(u.getAttribute("data-select-control"),{groups:e,prioritySet:s,scrollTree:!0,scrollDetail:!0})})})}function Tn(e,{groups:t,prioritySet:s,scrollTree:n=!1,scrollDetail:a=!1}={}){if(!e)return;const r=t||Oe(),o=Qt(i.analysis?.confirmationActions||[]),l=s||new Set(In(i.analysis?.confirmationActions||[]));i.pendingDoneEvidenceControlId&&i.pendingDoneEvidenceControlId!==e&&(i.pendingDoneEvidenceControlId=null),be(e);const d=r.find(f=>f.controls.some(v=>v.id===e));d&&i.sessionCollapsedCategories.delete(d.categoryId),F(e,o,l);const u=p("sessionMasterTree");u&&(u.querySelectorAll(".session-item").forEach(f=>{f.classList.toggle("is-selected",f.getAttribute("data-select-control")===e)}),u.querySelectorAll(".session-cat").forEach(f=>{const v=f.getAttribute("data-session-cat");f.classList.toggle("is-collapsed",i.sessionCollapsedCategories.has(v))}),n&&u.querySelector(`[data-select-control="${CSS.escape(e)}"]`)?.scrollIntoView({block:"nearest"}),a&&window.requestAnimationFrame(()=>{window.requestAnimationFrame(()=>{Ni(e)})}))}function Ni(e){const t=(e?document.querySelector(`.session-detail-card[data-today-control="${CSS.escape(e)}"]`):null)||document.querySelector(".session-detail-card")||p("sessionDetailPane");if(!t)return;(t.querySelector(".today-card-top")||t.querySelector(".today-question")||t).scrollIntoView({behavior:"smooth",block:"start"})}function Rn(e){return i.analysis?.controlSessionDetails?.[e]||null}function bs(e){return(e||[]).map(t=>`<li>${c(t)}</li>`).join("")}function Di(e,t,s){const n=(i.checklist||[]).find(M=>M.id===e)||{},a=Rn(e),r=L(e),o=t?.title||a?.title||n.title||"",l=t?.question||a?.question||`${o||e} 이행 상태를 확인했나요?`,d=String(t?.whyItMatters||"").trim(),u=String(t?.actionGuide||a?.actionGuide||"").trim(),f=d.length>220?`${d.slice(0,217)}…`:d,v=u.length>180?`${u.slice(0,177)}…`:u,g=W(e),h=Object.keys(V).map(M=>`
  ...
  `).join(""),b=n.checklistItems||[],A=Vt(e,b),E=b.map((M,Kn)=>{const mt=String(Kn+1),Jn=!!A[mt];return`
    ...
    `}).join(""),y=zt(e),x=i.pendingDoneEvidenceControlId===e,m=r==="evidenced"?"done":r,S=x?"done":m,w=Ai.map(M=>`
...
`);function ss(e){const t=String(e||"");return t.includes("내부 참고")||t.includes("전체 진행 반영")||t==="전체 진행 참고"||t==="전체 진행 반영 점수"?"전체 진행 참고":t.includes("평가분")||t.includes("점검분만")||t.includes("점검분 이행")||t==="점검분 이행 참고"||t==="점검분만 이행 점수"?"점검분 이행 참고":D(t)}function qn(e,t){const s=String(e?.label||""),n=ss(s);return n==="전체 진행 참고"||s.includes("내부 참고")||s.includes("전체 진행")?D(String(t?.overallScoreTooltip||ar).trim()):n==="점검분 이행 참고"||s.includes("평가분")||s.includes("점검분")?D(String(t?.assessedScoreTooltip||ir).trim()):D(String(e?.tooltip||"").trim())}function Bn(e={},t=null){const s=t||ie?.analysis||i.analysis,n=new Set(["전체 진행 참고","점검분 이행 참고"]),a=Array.isArray(e.stats)?e.stats.map(u=>{const f=ss(u?.label);let v=u?.value;if(n.has(f)){const g=String(v??""),h=g.match(/([\d.]+)\s*%/);v=h?$e(h[1]):D(g)}return{...u,label:f,value:v,tooltip:qn({...u,label:f},s)}}):e.stats,r=Array.isArray(e.basis)?e.basis.map(u=>D(u)):e.basis;let o=e.metric,l=e.metricUnit;const d=D(e.metricLabel);if(d==="점검분 이행 참고"||d==="전체 진행 참고"||String(e.metricUnit||"")==="%"){const u=Number(o);Number.isFinite(u)&&(d.includes("참고")||d.includes("점수"))&&(o=$e(u),l="")}return{...e,title:D(e.title),headline:D(e.headline),explanation:D(e.explanation),question:D(e.question),metricLabel:d,metric:o,metricUnit:l,body:D(e.body),nextAction:D(e.nextAction),stats:a,basis:r}}function As(e=[]){if(!e.length)return"";const t=ie?.analysis||i.analysis;return`
  ...
  `}function re(e){const t=p("reportReviewQueue");if(!t)return;vr(t);const s=pr(e);if(!s.length){t.dataset.shell="empty",ie=null,t.innerHTML=hr(e);return}const n=fr(e,s),a=Hn(n),r=new Set(a.confirmed),o=new Set(a.ignored),l=s.filter(b=>!r.has(b.id)&&!o.has(b.id)),d=s.length-l.length,u=l[0],f=s.length?Math.round(d/s.length*100):0,v=Math.min(d+1,s.length);if(!u){t.dataset.shell="complete",ie={analysis:e,fingerprint:n,current:a,active:null},t.innerHTML=`
    ...
    `}).join(""),s.querySelectorAll("[data-open-defect-evidence]").forEach(r=>{r.addEventListener("click",()=>{const o=e.find(l=>l.primaryControl?.controlId===r.dataset.openDefectEvidence)?.primaryControl;Ar(o)})})}}function Lr(e,{renderConfirmationActions:t}){const s=i.analysis;if(!s)return;if(!e){p("analyzeContent").style.display="";const m=p("analysisReportPanel");m&&(s.executiveReport||i.lastAiExecutiveReport)&&(m.style.display="",He(),re(s))}const n=s.areaCoverage||{},a=(s.categoryCoverage||s.weakCategories||[]).map(m=>{const S=Number(m.reviewedCount??0),w=Number(m.totalCount??m.count??0),k=Number.isFinite(Number(m.coveragePercent))?Number(m.coveragePercent):w>0?S/w*100:0,I=Qe(k);return{...m,reviewed:S,total:w,pct:I}}),r=a.reduce((m,S)=>m+S.reviewed,0),o=a.reduce((m,S)=>m+S.total,0),l=Qe(o>0?r/o*100:0),d=o>0&&r>=o,u=p("categoryCoverageSummary"),f=p("categoryCoverageList"),v=p("categoryListCount");if(v&&(v.textContent=`중분류 ${a.length}개 · 스크롤하여 전체 보기`),u&&(u.classList.toggle("is-complete",d),u.innerHTML=o?`
